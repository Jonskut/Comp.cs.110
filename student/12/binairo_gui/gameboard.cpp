...
{
    init();
    // Initialize the array with all values false
    for (int i = 0; i < SIZE; ++i)
    {
        for (int j = 0; j < SIZE; ++j)
        {
            illegal_moves_con[i][j] = false;
            illegal_moves_tot[i][j] = false;
        }
    }
}
...
bool GameBoard::ok_adjacent_symbols()
{
...
bool GameBoard::ok_amount_of_symbols()
{
    ...
    // Reset the entire illegal_moves_tot array to false
    ...
    for(unsigned int i = 0; i < board_.size(); ++i)
    {
        ...
        // Mark illegal moves in array
        if(zeros_in_row > NUMBER_OF_SYMBOLS)
        {
            for (unsigned int k = 0; k < SIZE; ++k)
            {
                if (board_.at(i).at(k) == ZERO)
                {
                    illegal_moves_tot[i][k] = true;
                }
            }
        }

        if(ones_in_row > NUMBER_OF_SYMBOLS)
        {
            for (unsigned int k = 0; k < SIZE; ++k)
            {
                if (board_.at(i).at(k) == ONE)
                {
                    illegal_moves_tot[i][k] = true;
                }
            }
        ...
        if(zeros_in_column > NUMBER_OF_SYMBOLS)
        {
            for (unsigned int k = 0; k < SIZE; ++k)
            {
                if (board_.at(k).at(i) == ZERO)
                {
                    illegal_moves_tot[k][i] = true;
                }
            }
        }

        if(ones_in_column > NUMBER_OF_SYMBOLS)
        {
            for (unsigned int k = 0; k < SIZE; ++k)
            {
                if (board_.at(k).at(i) == ONE)
                {
                    illegal_moves_tot[k][i] = true;
                }
            }
...
void GameBoard::set_symbol(unsigned int x, unsigned int y)
{
    switch(board_.at(y).at(x))
    {
        case EMPTY:
            board_.at(y).at(x) = ZERO; break;
        case ZERO:
            board_.at(y).at(x) = ONE; break;
        case ONE:
            board_.at(y).at(x) = EMPTY; break;
    }
...
void GameBoard::print_illegal_moves() const
{
    ...
    for(unsigned int i = 0; i < SIZE; ++i)
    {
        for(unsigned int j = 0; j < SIZE; ++j)
        {
            std::cout << (illegal_moves_con[i][j] ||
                          illegal_moves_tot[i][j] ? "X " : "O ");
        }
        std::cout << std::endl;
    }
}

bool GameBoard::is_illegal_move(int row, int col)
{
    return illegal_moves_con[row][col] || illegal_moves_tot[row][col];
}

// get board (for GUI)
const std::vector<std::vector<Element_type> > &GameBoard::get_board() const
{
    return board_;
}
...
bool GameBoard::ok_adjacent_syms_in_rows(Element_type elem)
{
    for(unsigned int i = 0; i < board_.size(); ++i)
    {
        for(unsigned int j = 0; j < board_.at(i).size() - 2; ++j)
        {
            ...
            {
                illegal_moves_con[i][j] = true;
                illegal_moves_con[i][j + 1] = true;
                illegal_moves_con[i][j + 2] = true;
                return false;
            }
            else
            {
                illegal_moves_con[i][j] = false;
                illegal_moves_con[i][j + 1] = false;
                illegal_moves_con[i][j + 2] = false;
            }
        }
    }
    return true;
...
bool GameBoard::ok_adjacent_syms_in_columns(Element_type elem)
{
    for(unsigned int i = 0; i < board_.size() - 2; ++i)
    {
        for(unsigned int j = 0; j < board_.at(i).size(); ++j)
        {
            ...
            {
                illegal_moves_con[i][j] = true;
                illegal_moves_con[i + 1][j] = true;
                illegal_moves_con[i + 2][j] = true;
                return false;
            }
            else
            {
                illegal_moves_con[i][j] = false;
                illegal_moves_con[i + 1][j] = false;
                illegal_moves_con[i + 2][j] = false;
            }
        }
    }
    return true;
