/*
 * Gameboard class that handles the backend of the game.
 * This program has minor modifications from the given template:
 *  - The symbol checking (starting with ok_) are updated to save
 *    illegal moves in an array
 *  - There is a method to check illegal moves array
 *  - Add symbol has been modified (You can add empty symbols)
 *  - Set symbol as a new method, changes the symbol based on the previous one.
 *  - Added a getter for the board.
 * Aforementioned modifications by:
...
public:
    ...
    // (Except when adding empty)
    // Returns true, if adding was done, otherwise returns false.
    bool add_symbol(unsigned int x, unsigned int y, char symbol_char);

    // Almost same as add_symbol, except it does not prevent from making
    // illegal moves. Also, allows setting symbol on all cells (not just empty)
    // Returns true if resulting board is legal.
    void set_symbol(unsigned int x, unsigned int y);
    ...
    // Method for debugging, prints illegal moves
    void print_illegal_moves() const;

    // Returns true if move at row, col is illegal
    bool is_illegal_move(int row, int col);

    // get board (for GUI)
    const std::vector<std::vector<Element_type>>& get_board() const;
...
private:
    ...
    // If this is the case, returns true, otherwise returns false.
    bool ok_adjacent_syms_in_rows(Element_type elem);
    ...
    // If this is the case, returns true, otherwise returns false.
    bool ok_adjacent_syms_in_columns(Element_type elem);

    // Gameboard
    std::vector<std::vector<Element_type>> board_;

    // Illegal moves consecutive
    bool illegal_moves_con[SIZE][SIZE];
    // Illegal moves total in row/col
    bool illegal_moves_tot[SIZE][SIZE];
