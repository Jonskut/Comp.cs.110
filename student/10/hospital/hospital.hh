...
private:
    std::map< std::string, std::shared_ptr<Person> > staff_;
    std::map< std::string, std::shared_ptr<Person> > current_patients_;
    std::map< std::string, std::shared_ptr<Person> > all_patients_;

    // Vector for storing care periods so it
    // can be accessed by order of insertion
    std::vector< std::pair< std::string,std::vector
            < std::shared_ptr<CarePeriod> > > > care_periods_;

    /*
     * For finding patient id iterator in care_periods_
     * param patient_id as string
     * return iterator for use in finding care period information
     */
    auto find_care_periods_by_id(const std::string& patient_id);
