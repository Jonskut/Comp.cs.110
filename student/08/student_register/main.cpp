...
bool save_data(const std::string& file_name,
               std::map< std::string, Student* >& alphabetical_order) {
    ...
    for(auto& student: alphabetical_order) {
        auto* s = student.second;
        ...
        if(command == "U" or command == "u") {
            ...
            if(found) {
                ...
                if(is_valid_phone_number(new_number)) {
                    student_numbers.at(parts[1])->phone_number = new_number;
