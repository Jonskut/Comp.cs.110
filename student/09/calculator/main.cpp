    ...
    while ( true ) {
        ...
        for(auto& command : COMMANDS)
        {
            if(command.str == command_to_be_executed)
            {
                ...
                {
                    bool non_number_operand = false;
                    vector<double> params(command.parameter_number);
                    for (unsigned int i = 1; i < pieces.size(); ++i)
                    {
                        if(!string_to_double(pieces.at(i), params.at(i-1)))
                        {
                            non_number_operand = true;
                        }
                        if (non_number_operand){
                            break;
                        }
                    }
                    if (non_number_operand)
                    ...
                    if(pieces.size() == 3)
                    {
                        double result = command.action(params[0], params[1]);
                        cout << result << endl;
                    }
                    else
                    {
                ...
                break; // No need to continue searching for commands
            }
        }
        if(!command_found){
